import { useState } from 'react'
import { Brain, Star, AlertTriangle, Loader2 } from 'lucide-react'
import { fetchPrediction } from '../services/api'

const RANK_COLORS = { 1: 'var(--rank-1)', 2: 'var(--rank-2)', 3: 'var(--rank-3)' }
const RANK_LABELS = { 1: '◎ 本命', 2: '○ 対抗', 3: '▲ 単穴' }

export default function AIPrediction({ raceId }) {
  const [prediction, setPrediction] = useState(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  const handleFetch = async () => {
    setLoading(true)
    setError('')
    try {
      const data = await fetchPrediction(raceId)
      setPrediction(data.prediction)
    } catch (e) {
      setError('AI予想の取得に失敗しました')
    } finally {
      setLoading(false)
    }
  }

  return (
    <div className="card" style={{ padding: 20, background: 'linear-gradient(135deg, rgba(20,31,20,0.9), rgba(10,20,10,0.95))' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div className="section-title" style={{ fontSize: 18 }}>
          <Brain size={18} />
          AI予想
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, color: 'var(--text-muted)' }}>
          <span style={{ padding: '2px 8px', borderRadius: 4, background: 'rgba(34,197,94,0.1)', color: 'var(--green-bright)', border: '1px solid rgba(34,197,94,0.2)' }}>
            Gemini 2.5 Flash
          </span>
        </div>
      </div>

      {!prediction && !loading && (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>
            出走馬の情報をAIが分析して予想を生成します
          </div>
          <button className="btn btn-gold" onClick={handleFetch} style={{ margin: '0 auto' }}>
            <Brain size={15} />
            AI予想を取得する
          </button>
        </div>
      )}

      {loading && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: '24px 0', color: 'var(--text-muted)' }}>
          <Loader2 size={28} style={{ animation: 'spin 1s linear infinite', color: 'var(--gold)' }} />
          <div style={{ fontSize: 13 }}>AIが分析中...</div>
        </div>
      )}

      {error && (
        <div style={{ color: '#f87171', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
          <AlertTriangle size={14} /> {error}
        </div>
      )}

      {prediction && !prediction.error && (
        <div className="animate-in">
          {/* 展開予想 */}
          {prediction.summary && (
            <div style={{ marginBottom: 16, padding: 14, borderRadius: 8, background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.15)' }}>
              <div style={{ fontSize: 11, color: 'var(--gold-dim)', fontWeight: 700, marginBottom: 6 }}>レース展開予想</div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{prediction.summary}</div>
            </div>
          )}

          {/* 予想上位3頭 */}
          {prediction.top3 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 10 }}>予想上位馬</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {prediction.top3.map((horse) => (
                  <div
                    key={horse.rank}
                    style={{
                      display: 'flex', gap: 12, padding: '10px 14px',
                      borderRadius: 8, background: 'var(--bg-secondary)',
                      border: '1px solid',
                      borderColor: horse.rank === 1 ? 'rgba(212,175,55,0.3)' : 'var(--border)',
                    }}
                  >
                    <div style={{ fontSize: 13, fontWeight: 800, color: RANK_COLORS[horse.rank], minWidth: 54 }}>
                      {RANK_LABELS[horse.rank]}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-primary)' }}>
                        {horse.horse_no}番 {horse.horse_name}
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3, lineHeight: 1.5 }}>
                        {horse.reason}
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          )}

          {/* 推奨買い目 */}
          {prediction.recommended_bet && (
            <div style={{ marginBottom: 12, padding: 14, borderRadius: 8, background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.15)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                <Star size={13} color="var(--green-bright)" />
                <span style={{ fontSize: 11, color: 'var(--green-bright)', fontWeight: 700 }}>推奨買い目</span>
                <span className="badge badge-green" style={{ marginLeft: 4 }}>{prediction.recommended_bet.type}</span>
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>
                {prediction.recommended_bet.combination}
              </div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>
                {prediction.recommended_bet.reason}
              </div>
            </div>
          )}

          {/* 免責 */}
          <div style={{ fontSize: 10, color: 'var(--text-muted)', display: 'flex', gap: 4, lineHeight: 1.5 }}>
            <AlertTriangle size={12} style={{ flexShrink: 0, marginTop: 1 }} />
            {prediction.caution || '本予想はAIによる参考情報です。馬券購入は自己責任でお願いします。'}
          </div>
        </div>
      )}

      {prediction?.error && (
        <div style={{ color: '#f87171', fontSize: 12 }}>{prediction.error}</div>
      )}
    </div>
  )
}
